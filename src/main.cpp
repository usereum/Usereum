...
uint256 hashGenesisBlock("0x0000031ec3c2a6ee208cfe2656e9f18c0135a8c410937cac2866073fcd8f0fdc");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Usereum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Usereum Signed Message:\n";
...
{
    // Usereum: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 1 * 60; // Usereum: each block
static const int64 nTargetSpacing = 1 * 60; // Usereum: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Usereum: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Usereum - DarkGravity, written by Evan Duffield - evan@Usereum.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Usereum: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Usereum: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Usereum";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc2a087c19ea51f3a04e72382d249fd11b515c7e23d9246190068293a6b59ef33"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // Usereum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UsereumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UsereumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UsereumMiner(CWallet *pwallet)
{
    printf("UsereumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Usereum-miner");
    ...
    try { while (true) {
        ...
        printf("Running UsereumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UsereumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UsereumMiner, pwallet));
}
